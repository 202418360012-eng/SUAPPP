...
async def on_fetch(request, env):
    try:
        ...
        if path == '/processar' and request.method == 'POST':
            ...
            login_page_res = await fetch(login_url)
            ...
            if not csrf_match:
                return Response.new(
                    json.dumps({"erro": "Não foi possível obter o token CSRF do SUAP."}), 
                    status=500, 
                    headers=Headers.new({'Content-Type': 'application/json'})
                )
            ...
            post_res = await fetch(login_url, {
                'method': 'POST',
            ...
            if "Usuário ou senha inválidos" in post_html:
                return Response.new(
                    json.dumps({"erro": "Matrícula ou senha incorretas."}), 
                    status=401, 
                    headers=Headers.new({'Content-Type': 'application/json'})
                )
            ...
            boletim_res = await fetch(boletim_url, {'headers': boletim_headers})
            ...
            for tr in tr_blocks:
                ...
                if 'disciplina' in cols[0].lower() or 'disciplina' in cols[1].lower():
                    continue
                ...
                alerta = "🚨 *ESTOURADO!*" if faltas_restantes < 0 else ("⚠️ *ATENÇÃO!*" if faltas_restantes <= 2 else "✅ *OK*")
                texto_mensagem += f"🔹 *{disciplina}*\n   • Faltas: {faltas_atuais}/{limite_max_faltas}\n   • Restantes: *{faltas_restantes}* {alerta}\n\n"

            if not dados_materias:
                return Response.new(
                    json.dumps({"erro": "Nenhuma disciplina encontrada."}), 
                    status=404, 
                    headers=Headers.new({'Content-Type': 'application/json'})
                )

            return Response.new(
                json.dumps({'materias': dados_materias, 'mensagem': texto_mensagem, 'telefone': telefone}), 
                status=200, 
                headers=Headers.new({'Content-Type': 'application/json'})
            )

        # Retorna a interface web HTML caso seja acesso normal (GET /)
        return Response.new(
            HTML_PAGE, 
            status=200, 
        ...
        return Response.new(
            f"Erro interno no Worker: {str(err)}", 
            status=500, 
